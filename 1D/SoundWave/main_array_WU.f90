...
integer,parameter::nx=128*2*2*2      ! the number of grids in the simulation box
...
!         write(1,*) time, 0.5d0*(Q(2,IDN) + Q(3,IDN))
      ...
      error = 0.0d0
      do i=is,ie
         error = error + (Q(i,IDN) - 1.0d0 - 1.0d-4*dsin(2.0d0*dacos(-1.0d0)*x1c(i)))**2
      enddo
      print*,nx,dsqrt(error/nx )
      call Output( x1b, x1c, Q)
!      do i=is,ie
!         print*,x1c(i),Q(i,IDN),1.0d0 + 1.0d-4*dsin(2.0d0*dcos(-1.0d0)*x1c(i))
!      enddo
!      close(1)
...
real(8)::pi,amp
      ...
      do i=is,ie
             Q(i,IDN) = 1.0d0 + amp*dsin(2.0d0*pi*x1c(i))
             Q(i,IVX) = amp*dsqrt(gam)*dsin(2.0d0*pi*x1c(i))
             Q(i,IPR) = 1.0d0 + amp*gam*dsin(2.0d0*pi*x1c(i))
      enddo
...
!            U(i,n) = U(i,n) + dt*(- F(i+1,n) + F(i,n))/(x1b(i+1)-x1b(i)) 
            U(i,n) = U(i,n) + dt*(- F(i,n) + F(i-1,n))/(x1b(i)-x1b(i-1)) 
        enddo
    enddo
