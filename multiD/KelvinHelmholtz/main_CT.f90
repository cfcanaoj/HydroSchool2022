...
!      write(6,*) "setup grids and initial condition"
      ...
      call Output( xf, xv, yf, yv, Q, Bc )



      open(1,file="vy_evo_B0.8_ct1.dat",action="write")
! main loop
...
!---------------------------------------------------------------------
      ...
      real(8), intent(inout)  :: weight1(:,:,:)
      real(8), intent(inout)  :: weight2(:,:,:)
      real(8), intent(out) :: E(:,:,:,:)
...
!          E(i,j,k,3) = 0.5d0*( e3_yf(i-1,j,k) + e3_yf(i,j,k) + e3_xf(i,j-1,k) + e3_xf(i,j,k)) &
!                     - 0.25d0*( Etmp(i-1,j-1,k) + Etmp(i-1,j,k) + Etmp(i,j-1,k) + Etmp(i,j,k) )
...
!!=====================================================================
      ...
      subroutine Output( xf, xv, yf, yv, Q, Bc )
      use commons
      implicit none
      real(8), intent(in) :: xf(:), xv(:), yf(:), yv(:)
      real(8), intent(in) :: Q(:,:,:,:), Bc(:,:,:,:)
      integer::i,j,k
      character(20),parameter::dirname="snap_B0.8_ct1"
