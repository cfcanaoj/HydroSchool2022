...
integer,parameter::nx=150*1*1        ! the number of grids in the simulation box
integer,parameter::ny=150*1*1 ! the number of grids in the simulation box
...
!!$omp end parallel
  print *, "END"
      ...
      open(unitevo,file="cpu.dat",action="write")
!  stime = omp_get_wtime()
      ntime = 1
...
!---------------------------------------------------------------------
      ...
      real(8),dimension(in,NFLX):: Ql0,Qr0,flx0
...
!$omp parallel 
      ...
      do j=js,je
         do i=is-1,ie+1
            ...
            Ql0(i+1,1:NVAR) = Q(i,j,k,1:NVAR) + 0.5d0*dQmon(1:NVAR)
            Qr0(i  ,1:NVAR) = Q(i,j,k,1:NVAR) - 0.5d0*dQmon(1:NVAR)
         enddo
   
            call HLLD0(1,Ql0,Qr0,flx0)
         do i=is-1,ie+1
              F(i,j,k,:)  = flx0(i,:)
         enddo
      enddo
...
!            index: (IDN, IV1, IV2, IV3, IPR, IBperp1, IBperp2)
!---------------------------------------------------------------------
      subroutine HLLD0(idir,Ql0,Qr0,flx0)
      implicit none
      integer, intent(in) :: idir
      real(8),intent(in)  ::Ql0(:,:), Qr0(:,:)
      real(8),intent(out) :: flx0(:,:)
      real(8) :: Ql(NFLX), Qr(NFLX), flx(NFLX)
      integer :: IVpara, IVperp1, IVperp2
      integer :: IBpara, IBperp1, IBperp2
      real(8):: b1
      real(8):: Ul(NFLX), Ur(NFLX)
...
!         print*,test(IVperp2)
    ...
    !--- Step 6.  Compute flux
          if( S0 >= 0.0d0 ) then
           ...
           flx(IB1) = flag_HDC*0.5d0*(Ql(IPS) + Qr(IPS) - ch*(Qr(IB1) - Ql(IB1)) )
           flx(IPS) = flag_HDC*0.5d0*(ch*ch*(Ql(IB1) + Qr(IB1)) - ch*(Qr(IPS) - Ql(IPS)) )
...
!
      ...
      character(20),parameter::dirname="openmp_test_4cpu"
