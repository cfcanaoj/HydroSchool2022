...
real(8) :: pi, den, B0, rho1, rho2, dv, wid, sig
      ...
      do i=is,ie
           ...
           Q(IV2,i,j,k)= 0.01d0/4.0d0 &
                     & *(-dcos(2.0d0*pi*(xv(i)-(xmax+xmin)/2.0d0)/(xmax-xmin))) &
                     & *(1.0d0+cos(2.0d0*pi*(yv(j)-(ymax+ymin)/2.0d0)/(ymax-ymin)))
!                     & *dexp( - (xv(i) - (xmax+x1min)/2.0d0)**2/(0.1**2) )
!                     & *(+cos(2.0d0*pi*(xv(i)-(xmax+x1min)/2.0d0)/(xmax-x1min)))
      enddo
      enddo
      enddo
...
integer::i,j,k,ish
      ...
      do i=1,nxtot-1
          ...
          Q(IV1,i,js-j,k)  = Q(IV1,i,js-1+j,k)
          Q(IV2,i,js-j,k)  = -Q(IV2,i,js-1+j,k)
          Q(IV3,i,js-j,k)  = Q(IV3,i,js-1+j,k)
          Q(IPR,i,js-j,k)  = Q(IPR,i,js-1+j,k) &
                           - Q(IDN,i,js-1+j,k)*grav_accy*(2*j-1)*(yf(j+1)-yf(j))
          Q(IB1,i,js-j,k)  = Q(IB1,i,js-1+j,k)
          Q(IB2,i,js-j,k)  = Q(IB2,i,js-1+j,k)
          Q(IB3,i,js-j,k)  = Q(IB3,i,js-1+j,k)
      ...
      do i=1,nxtot-1
          ...
          Q(IV1,i,je+j,k) = Q(IV1,i,je-j+1,k)
          Q(IV2,i,je+j,k) = Q(IV2,i,je-j+1,k)
          Q(IV3,i,je+j,k) = Q(IV3,i,je-j+1,k)
          Q(IPR,i,je+j,k) = Q(IPR,i,je-j+1,k) &
                          + Q(IDN,i,je-j+1,k)*grav_accy*(2*j-1)*(yf(j+1)-yf(j))
          Q(IB1,i,je+j,k) = Q(IB1,i,je-j+1,k)
          Q(IB2,i,je+j,k) = Q(IB2,i,je-j+1,k)
          Q(IB3,i,je+j,k) = Q(IB3,i,je-j+1,k)
...
!$omp parallel 
      ...
      do i=is,ie
         ...
         U(IM2,i,j,k) = U(IM2,i,j,k) + src
         U(IEN,i,j,k) = U(IEN,i,j,k) + src*Q(IV2,i,j,k)
        ...
        write(unitsnap,*) "#nx, ny = ", nx, ny
          ...
          do i=is,ie
              write(unitsnap,*) xv(i), yv(j), Q(IDN,i,j,k), Q(IV1,i,j,k), Q(IV2,i,j,k), Q(IV3,i,j,k), &
                                Q(IPR,i,j,k), Q(IB1,i,j,k), Q(IB2,i,j,k), Q(IB3,i,j,k), Q(IPS,i,j,k) 
          enddo
          enddo
          enddo
          close(unitsnap)
