...
!         write(*,*) time, errorBpara(Q), divergenceB(x1a,x2a,Bc)
...
!      sina = 0.0d0
      ...
      do j=js,je+1
      do i=is,ie
!          Bs(i,j,k,2) = sina*BxR + cosa*ByR
        if ( x1b(i)*cosa + x2a(j)*sina - x1m< 0.0d0 ) then
          Bs(i,j,k,2) = sina*BxL + cosa*ByL
        else 
          Bs(i,j,k,2) = sina*BxR + cosa*ByR
        endif
      enddo
...
!      call BoundaryCondition
      ...
      do j=js-mgn,je+mgn
      do i=1,mgn
          Bs(ie+i+1,j,k,1) = Bs(ie-i+1,j,k,1)
      enddo
      enddo
      enddo
...
!          ish = i
      ...
      do i=is-mgn,ie+mgn+1
          ...
          Bs(i,js-j,k,1) = Bs(ish,js+j-1,k,1)
      enddo
      enddo
      enddo
      ...
      do i=is-mgn,ie+mgn
          ...
          Bs(i,js-j,k,3) = Bs(ish,js+j-1,k,3)
      enddo
      enddo
      enddo
...
!         ish = i
      ...
      do i=is-mgn,ie+mgn+1
          ...
          Bs(i,je+j,k,1) = Bs(ish,je-j+1,k,1)
      enddo
      enddo
      enddo
      ...
      do i=is-mgn,ie+mgn
          ...
          Bs(i,je+j+1,k,2) = Bs(ish,je-j+1,k,2)
      enddo
      enddo
      enddo
      ...
      do i=is-mgn,ie+mgn
          ...
          Bs(i,je+j,k,3) = Bs(ish,je-j+1,k,3)
      enddo
      enddo
      enddo
...
!
      ...
      do i=is,ie
             Bc(i,j,k,1) = 0.5d0*( Bs(i+1,j,k,1) + Bs(i,j,k,1) )
             Bc(i,j,k,2) = 0.5d0*( Bs(i,j+1,k,2) + Bs(i,j,k,2) )
             Bc(i,j,k,3) = 0.5d0*( Bs(i,j+1,k,3) + Bs(i,j,k,3) )
      enddo
      enddo
      enddo
...
!      do j=js,je-1
!      do i=is,ie
!      print*, "e3 ",e3_x1a(i,j+1,k) - e3_x1a(i,j,k),e3_x2a(i,j+1,k) - e3_x2a(i,j,k)
!
!      enddo
!      enddo
!      enddo
...
!        if(n==1) print*,x1b(i),- F(i+1,j,k,n) + F(i,j,k,n)
      ...
      do j=js,je
      do i=is,ie+1
           Bs(i,j,k,1) = Bso(i,j,k,1) - dt1*(E(i,j+1,k,3) - E(i,j,k,3))/(x2b(j+1) - x2b(j))
      enddo
      enddo
      enddo
      ...
      do j=js,je+1
      do i=is,ie
           Bs(i,j,k,2) = Bso(i,j,k,2) + dt1*(E(i+1,j,k,3) - E(i,j,k,3))/(x1b(i+1) - x1b(i))
      enddo
      enddo
      enddo
