...
!---------------------------------------------------------------------
      ...
      do i=is-1,ie+1
         ...
         Wl(i+1,j,k,IB1:IB3) = B(i,j,k,1:3) + 0.5d0*dWmon(IB1:IB3)*1.0d0
         Wr(i  ,j,k,IB1:IB3) = B(i,j,k,1:3) - 0.5d0*dWmon(IB1:IB3)*1.0d0

         Wl(i+1,j,k,IPH) = phi(i,j,k) + 0.5d0*dWmon(IPH)*1.0d0
         Wr(i  ,j,k,IPH) = phi(i,j,k) - 0.5d0*dWmon(IPH)*1.0d0
      enddo
      enddo
      enddo
      ...
      do i=is,ie
         ...
         Wl(i,j+1,k,IB1:IB3) = B(i,j,k,1:3) + 0.5d0*dWmon(IB1:IB3)*1.0d0
         Wr(i,j  ,k,IB1:IB3) = B(i,j,k,1:3) - 0.5d0*dWmon(IB1:IB3)*1.0d0

         Wl(i,j+1,k,IPH) = phi(i,j,k) + 0.5d0*dWmon(IPH)*1.0d0
         Wr(i,j  ,k,IPH) = phi(i,j,k) - 0.5d0*dWmon(IPH)*1.0d0
      enddo
      enddo
      enddo
...
!          Ust(:) = ( sr*Ur(:) - sl*Ul(:) - Fr(:) + Fl(:) )/(sr - sl)
          ...
          flx(IBpara) = 0.5d0*(Wl(IPH) + Wr(IPH)) - 0.50d0*ch*(Wr(IBpara) - Wl(IBpara))
          flx(IPH) = ch*ch*( 0.5d0*( Wl(IBpara) + Wr(IBpara) ) - 0.5d0/ch*(Wr(IPH) - Wl(IPH)) )
!          flx(IBpara) = 0.0d0
!          flx(IPH) = 0.0d0
...
!      open(unitbin,file=filename,status='replace',form='formatted') 
      ...
      write(unitbin,"(a9,f4.2)") "# time = ",time
      do k=ks,ke
      do j=js,je
      do i=is,ie
